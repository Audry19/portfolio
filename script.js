...
function initShowcase() {
  ...
  function centerWindowOn(i) {
    // Center the window: 2 on each side (desktop) or 1-2 on each side (mobile)
    const offset = isMobile ? 1 : 2;
    let newStart = i - offset;
  ...
  function setActive(i, fromHover) {
    if (fromHover && hoverLocked) return;
    items.forEach(el => el.classList.remove('active'));
    items[i].classList.add('active');
    activeIndex = i;
    updateText(i);
    updateCounter();
    // Lock hover briefly when window shifts to prevent chain reaction
    const oldStart = windowStart;
    centerWindowOn(i);
    if (oldStart !== windowStart && fromHover) {
      hoverLocked = true;
      setTimeout(() => { hoverLocked = false; }, 300);
    }
  }
...
function hideSplineBadge() {
  ...
  function nukeWatermark(sr) {
    // Remove #logo
    const logo = sr.querySelector('#logo');
    if (logo) logo.remove();

    // Remove any spline links
    sr.querySelectorAll('a').forEach(a => {
      if (a.href && a.href.includes('spline')) a.remove();
    });

    // Remove by class/id patterns
    sr.querySelectorAll('[class*="logo"], [class*="watermark"], [class*="badge"], [id*="logo"], [id*="watermark"]').forEach(el => el.remove());

    // Remove "Built with Spline" or any small overlay text
    sr.querySelectorAll('div, button, span, a').forEach(el => {
      const txt = (el.textContent || '').trim();
      if (
        (txt.includes('Built with') || txt.includes('Spline') || txt.includes('Made with')) &&
        el.offsetHeight < 100 && el.offsetWidth < 300
      ) {
        el.remove();
      }
    });

    // Kill any small fixed/absolute positioned element at the bottom (watermark pattern)
    sr.querySelectorAll('*').forEach(el => {
      const style = getComputedStyle(el);
      if (
        (style.position === 'absolute' || style.position === 'fixed') &&
        el.offsetHeight < 80 && el.offsetHeight > 0 &&
        el.offsetWidth < 300 && el.offsetWidth > 0
      ) {
        const rect = el.getBoundingClientRect();
        const parentRect = viewer.getBoundingClientRect();
        // If it's near the bottom of the viewer, it's the watermark
        if (rect.bottom >= parentRect.bottom - 60) {
          el.remove();
        }
      }
    });
  ...
  // Poll until shadow root is available, then attach a permanent observer
  const poll = setInterval(() => {
    ...
    // Permanent observer — never disconnects, catches any re-injection
    const observer = new MutationObserver(() => {
      nukeWatermark(sr);
    });
    observer.observe(sr, { childList: true, subtree: true });

    // Also re-run periodically in case mutations don't fire
    setInterval(() => nukeWatermark(sr), 2000);
  }, 150);
}
